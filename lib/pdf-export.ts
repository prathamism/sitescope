...
export async function exportAuditPDF(result: AuditResult): Promise<void> {
  ...
  function newPage() {
    ...
    doc.text(`RANKLENS AUDIT  ·  ${result.domain}`, ML, 5.5)
  ...
  for (let p = 1; p <= pageCount; p++) {
    ...
    doc.setFont('helvetica', 'normal')
    doc.setTextColor(...C.muted)
    doc.text('Generated by RankLens  ·  ranklens.app  ·  Free SEO + GEO auditing', ML, H - 3)
    doc.text(`${p} / ${pageCount}`, W - MR, H - 3, { align: 'right' })
  }

  doc.save(`ranklens-${result.domain}-report.pdf`)
}
...
export async function exportComparePDF(compare: import('./types').CompareResult): Promise<void> {
  ...
  function newPage() {
    ...
    doc.text(`RANKLENS  ·  ${compare.siteA.domain} vs ${compare.siteB.domain}`, ML, 5.5)
    curY = 14
  }
  ...
  for (let p = 1; p <= pageCount; p++) {
    ...
    doc.text('Generated by RankLens  ·  ranklens.app', ML, H - 3)
    doc.text(`${p} / ${pageCount}`, W - MR, H - 3, { align: 'right' })
  }

  doc.save(`ranklens-compare-${siteA.domain}-vs-${siteB.domain}.pdf`)
}
...
export async function exportBulkPDF(results: import('./types').BulkResult[]): Promise<void> {
  ...
  function newPage() {
    ...
    doc.setFillColor(...C.surface); doc.rect(0, 0, W, 8, 'F')
    doc.setFontSize(7); doc.setFont('helvetica', 'normal'); doc.setTextColor(...C.muted)
    doc.text('RANKLENS  ·  BULK AUDIT REPORT', ML, 5.5)
    curY = 14
  }
  function ensureSpace(n: number) { if (curY + n > H - 14) newPage() }
  function divider() {
  ...
  doc.save(`ranklens-bulk-${results.length}sites.pdf`)
}
